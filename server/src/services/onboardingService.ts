...
export const getOnboardingStatus = async (userId: string) => {
  const result = await query(
    'SELECT is_onboarding, role FROM users WHERE id = $1',
    [userId]
  );
...
export const completeInfluencerOnboarding = async (
  userId: string,
  data: { username: string; socialAccounts?: Array<{ platform: string; username: string; profileUrl?: string }> }
) => {
    ...
    if (data.socialAccounts?.length) {
      ...
      for (const account of data.socialAccounts) {
        if (account.platform && account.username && account.username.trim()) {
          await client.query(
            'INSERT INTO social_accounts (profile_id, platform, username, profile_url) VALUES ($1, $2, $3, $4)',
            [profileId, account.platform.toUpperCase(), account.username.trim(), account.profileUrl || null]
          );
        }
      }
    }
...
export const completeAgencyOnboarding = async (
  userId: string,
  data: { companyName: string; logoUrl?: string }
) => {
  const client = await getClient();
  try {
    await client.query('BEGIN');
    ...
    await client.query(
      'UPDATE profiles SET company_name = $1, logo_url = $2 WHERE user_id = $3',
      [data.companyName.trim(), data.logoUrl || null, userId]
    );
